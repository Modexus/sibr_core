...
def main():
    ...
    with open(os.path.join(os.path.abspath(os.path.dirname(__file__)), "ColmapQualityParameters.json")) as qualityParamsFile:
        qualityParams = json.load(qualityParamsFile)

        for key in qualityParams:
            if key not in args or args[key] is None:
                args[key] = qualityParams[key][args["quality"]] if args["quality"] in qualityParams[key] else qualityParams[key]["default"]

    # Get process steps
    with open(os.path.join(os.path.abspath(os.path.dirname(__file__)), "SelectiveColmapProcessSteps.json")) as processStepsFile:
        ...
        if( to_step != "default" ):
            ...
            for s in steps:
                ...
                if( s["name"] == to_step and s["name"] != "use_eol_fixed_mesh" ):
                    break
                if adding_steps and s["name"] not in exclude_steps :
                    newsteps.append(s)
            steps = newsteps
        else:
